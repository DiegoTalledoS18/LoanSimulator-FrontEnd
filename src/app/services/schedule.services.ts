...
export class ScheduleService {
  ...
  // Endpoint Backend
  basePath = 'http://localhost:8080/api/v1/schedule';
  ...
  getAll(): Observable<Schedule> {
    return this.http.get<Schedule>(`${this.basePath}`, this.httpOptions)
      .pipe(
        retry(2),
  ...
  getById(id: Number): Observable<Schedule> {
    return this.http.get<Schedule>(`${this.basePath}/${id}`, this.httpOptions)
      .pipe(
        retry(2),
        catchError(this.handleError));
  }
